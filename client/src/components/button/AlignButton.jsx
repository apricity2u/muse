...
export default function AlignButton({ clickHandler1, clickHandler2, selected="최신순" }) {
  return (
    <>
      <button onClick={clickHandler1} className={selected === "최신순" ? styles.selected : styles.button}>
        최신순
      </button>

      <button onClick={clickHandler2} className={selected === "좋아요순" ? styles.selected : styles.button}>
        좋아요순
      </button>
    </>
  );
